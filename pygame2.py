...
canvas = Canvas(window, width=1600, height=1400, bg = 'black')
canvas.pack()
...
def make_candy():
    ...
    xposition = random.randint(1, 1600)
    # pick a random color
...
def end_game_over():
#   window.destroy()
    game_restart = canvas.create_text(800, 500, text= 'press space bar to play again', fill='white', font = ('Helvetica', 50))
    pass
...
def check_hits():
    ...
    for candy in bad_candy_list:
        if collision(mychar, candy, 30):
            game_over = canvas.create_text(800, 400, text= 'Game Over', fill='red', font = ('Helvetica', 90))
...
def move_character():
    if move_direction == "Right" and canvas.coords(mychar)[0] < 1600:
    ...
    if move_direction == "Down" and canvas.coords(mychar)[1] < 1400 :
        canvas.move(mychar, 0,10)
